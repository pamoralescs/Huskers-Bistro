...
function SoupSaladPage() {
    const [data, setData] = useState([]);

    useEffect(() => {
        axios.get('http://localhost:5064/api/order/getallitems')
            .then((result) => {
                setData(result.data.filter(item => item.category === ('SoupSalad')));
            })
            .catch((error) => {
                console.log(error);
            })
    ...
    return (
        <div>
            ...
            <Container fluid className='mt-4'>
                <h2 className='mb-4 text-center' style={{ textDecoration: 'underline', fontWeight: 'bold' }}>Soup and Salads</h2>
                <Row className="g-4">
                    {
                        data && data.length > 0 ?
                            data.map((item) => {
